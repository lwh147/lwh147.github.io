...
当 Context Switch 发生时，需要有操作系统保存当前线程的状态，并恢复另一个线程的状态，Java 中对应的概念就是程序计数器（Program Counter Register），它的作用是记住下一条 jvm 指令的执行地址，是线程私有的
...
两阶段终止模式（Two Phase Termination）指的是在一个线程 `T1` 中如何“优雅”终止线程 `T2` ？这里的“优雅”指的是给 `T2` 一个料理后事的机会
