...
当Mapper接口方法的参数只有一个时，可以通过标签 `parameterType` 属性指定该参数的类型，然后进行引用， `parameterType` 主要分为三种类型：

* 基本包装数据类型（`String`、`Integer`、`Long`等）
* JavaBean
* Map

对于基础数据类型采用 `#{参数名}` 获取参数的内容，可以使用 `@Param` 注解给参数起别名 对于JavaBean采用 `#{属性名}` 或 `#{参数名.属性名}` ，由于只有一个参数，大可不必指定参数名，一般也就不需要起别名
