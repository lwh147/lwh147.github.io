...
当 `n` 为偶数时，可以表示为： `n = 1 * 2^x (x > 0)` ，且在二进制下的 `n` 的第 `x + 1` 位为 `1` ，其它位均是 `0` ，在这个例子中 `n = 16 = 1 * 2^4` ，所以 `x = 4` ， `x + 1 = 5` ，此时如果将 `x + 1 = 5` 作为高低位分界线，那么 `hash = 83` 可以表示为高位部分（位次大于等于 `5` 的部分）的值加上低位部分（位次小于 `5` 的部分 ）的值，即 `hash = (1*2^6 + 1*2^4) + (1*2^1 + 1*2^0) = (64 + 16) + (2 + 1)` ，将高位部分提出一个公因子 `2^4 = 16` 可得 `hash = 2^4(2^2 + 1) + (2^1 + 2^0) = 16(4 + 1) + (2 + 1) = 16*5 + 3` ，很明显低位部分的值 `3` 就是余数

上面只是一个特定的例子，但是是可以推广的，对于任意 `hash` 值（整数），转换为二进制后的高位部分（位次大于等于 `x + 1` ）均可以整除 `n` （因为高位部分所有位权都含有公因子 `n` ；高位部分如果均为 `0` 说明 `hash < n` ， `hash` 本身就是余数，但也遵守该规律），而 `n - 1` 也会得到一个低位部分（位次小于 `x + 1` ）均为 `1` ，高位部分均为 `0` 的数（因为 `n` 是偶数），该数和 `hash` 进行按位与运算自然结果就是 `hash` 低位部分的值，也就是余数，并且，因为 `n` 必定为正整数，所以 `n` 的符号位必定为 `0` ，所以使用按位与运算得到的余数的符号位也必定是 `0` ，也就是说这样计算余数会自动取正（真的是太巧妙了）

那么经过怎样的运算可以直接得到低位部分的值呢？ 这就是 `HashMap` 取余运算设计非常巧妙的地方了，我们将 `n - 1` ，那么就会变成这样：
...
我们得到了一个低位部分全都是 `1` 且高位全都是 `0` 的数，然后结合**按位与**运算的特性，将 `n-1` 和 `hash` 进行按位与运算就可以得到 `hash` 低位部分的值
...
在上面的推广结论中，最后使用一个低位部分均为 `1` ，高位部分均为 `0` 的数 `n-1` 和 `hash` 进行按位与运算得到了余数（下标），问题就出在这里（其实你仔细想想就能明白了），因为一旦模 `n` 确定了之后（容量确定之后），不论 `hash` 的值是多少，最终运算的结果只取决于 `hash` 的低位部分（位次小于 `x + 1` 的部分），也就是说 `hash` 值的低 `x` 位就能够决定最后的余数（下标）是多少， `hash` 的值虽然是随机变化的，但只要低位部分相同的 `hash` ，它们计算的结果就相同

理解了为什么使用偶数会提高哈希冲突概率的原因之后要解决这个问题的思路就非常清晰了，只需要让 `hash` 的低位部分也能够反映高位部分的取值情况即可，也就是说不能直接使用原始的 `key` 的 `hash` 值进行哈希计算下标，需要将其进行某种运算之后得到符合条件的 `hash1` 之后进行计算，在 `HashMap` 中这个过程称之为对 `hash` 值的**扰动**，得到 `hash1` 的具体计算过程就是**扰动算法**，下面来看看 `HashMap` 的扰动算法：
...
答：这里使用无符号右移和有符号右移都是可以的，但是**无符号右移速度更快**（不用考虑符号位情况，无脑补 `0` 即可），并且，这里无需考虑计算结果的正负问题（后面计算下标时会自动取正），所以使用无符号右移

2、为什么右移16位？不能是15、17位？

答：作者说一下自己的理解吧（仅供参考），在实际使用过程中，哈希表的容量是随机的，也就是说决定哈希码的低位部分位数是随机的，所以扰动时需要尽可能的让低位部分位数更多（尽可能让可扰动的哈希表容量上限更大），但同时又需要低位部分都能反应高位部分的信息（低位部分每位都要有与之对应的高位并且这两部分不能交叉），所以满足条件的低位部分最多只能是容量位数上限的一半，在32位的情况下也就是16位，所以右移16位，并且对于容量大于 `2^16` 的情况，哈希冲突的概率已经足够小了，已经可以不需要扰动了

> 不能交叉是指在容量确定时，低位部分与低位部分、高位部分与高位部分自身进行了关联运算，比如只右移 `2` 位、容量为 `n = 2 ^ 4 = 16` 时，低位部分（x <= 4）中的第1位、第2位会和同处于低位部分的第3位和第4位进行异或运算产生关联，高位部分（x > 4）中的第5位、第6位会和同为高位部分的第7位、第8位进行异或运算产生关联等

3、为什么要用异或运算？不能用与、或、同或运算吗？

答：首先，使用同或是可以的，但是在Java中只有异或运算符（ `^` ），同或只能通过对异或的结果按位非（ `~` ）后得到，所以就直接使用了异或；其次，与、或运算会分别使运算结果向 `0` 、 `1` 靠拢（总共有四种运算情况： `00` ， `01` ， `10` ， `11` ，与运算时四种情况中有三种情况结果都是 `0` ，或运算中有三种情况结果都是 `1` ），从而使计算结果靠拢，增加了哈希冲突的概率，违背了扰动的初衷
...
4、建立公共溢出区法：非常简单，就是将哈希表分为基本表和溢出表两个部分，把存在冲突的元素统一放到溢出表中即可

`HashTable` 使用的就是链地址法，没什么特别的； `HashMap` 虽然也使用了链地址法，但在同一冲突元素数量大于等于常量 `TREEIFY_THRESHOLD = 8` 且哈希表容量大于等于常量 `MIN_TREEIFY_CAPACITY = 64` 时，会将单向链表转变为红黑树，因为哈希表追求的查询速度是 `O(1)` ，当冲突元素过多时只使用单链表会使查询速度减慢到 `O(n)` ，而使用了红黑表会使查询速度又变为 `O(log n)` ，在元素被删除减少到小于等于常量 `UNTREEIFY_THRESHOLD = 6` 时，红黑树又会转换为单链表
