...
var springLoader = {
  ...
  initRebound: function initRebound() {
    ...
    var settings = springLoader.settings.rebound;

    // Create a SpringSystem.
    var springSystem = new rebound.SpringSystem();

    // Add a spring to the system.
    springLoader.spring = springSystem.createSpring(settings.tension, settings.friction);
  },
  ...
  initSpinner: function initSpinner() {
    ...
    var settings = springLoader.settings.spinner;

    // Instantiate Spinner.
    springLoader.spinner = new Spinner(settings);
  },


  /**
   * Initialize springLoader.
   */
  init: function init() {
    ...
    springLoader.initRebound();
    springLoader.initSpinner();

    // Init animation with Rebound Spring System.
    springLoader.spinner.init(springLoader.spring, spinnerTypeAutoSpin);

    if (spinnerTypeAutoSpin) {
      // Fake loading time, in a real world just call springLoader.spinner.setComplete();
      // whenever the preload will be completed.
      setTimeout(function () {
        springLoader.spinner.setComplete();
      }, 6000);
    } else {
      // Perform real ajax request.
      springLoader.loadSomething();
    }
  },
  ...
  loadSomething: function loadSomething() {
    ...
    oReq.addEventListener('progress', function (oEvent) {
      if (oEvent.lengthComputable) {
        ...
        springLoader.spring.setEndValue(percent * 0.01);
      }
    });
...
//# sourceMappingURL=spring-loader.js.map
