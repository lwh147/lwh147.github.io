...
> 引申：[SpringCloud Alibaba——Nacos配置中心配置优先级](https://blog.csdn.net/li1325169021/article/details/131743194)
...
**1. 前端发送请求，后端成功接收到数据，但是前端报404错误**

Controller没有打 `@RequestBody` 注解，导致SpringBoot认为Controller返回值是一个视图，找不到对应的视图所以报404

**2. A component required a bean of type 'xxx' that could not be found**
...
**3. SpringBoot整合Thymeleaf项目，配置和路径等都无误但是访问视图404**

检查SpringBoot版本和Thymeleaf版本是否匹配

**4. SpringApplicationRunListener在SpringBoot项目启动过程中执行了两次**

> 参考：[SpringApplicationRunListener在SpringBoot项目启动过程中执行了两次](https://blog.csdn.net/NEWCIH/article/details/129893523)
