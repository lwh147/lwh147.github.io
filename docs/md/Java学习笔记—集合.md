...
**自然排序**：实现了Comparable接口，所以TreeSet可以调用对象的ComparableTo()
...
首先，作为 `key` 的类必须重写 `Object` 类的 `hashCode()` 和 `equals()` 方法（原因自行了解），而这个方法的返回值类型为 `int`
，所以 `hash` 有可能为负数：
...
解决这个问题的思路也非常清晰，既然低位部分能够决定最后的余数，那只需要让低位部分也能够反映高位部分的取值情况即可，也就是说不能直接使用原始的 `hash`
